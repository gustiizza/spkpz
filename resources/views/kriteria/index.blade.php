                                    ...
                                    @foreach ($kriteria as $krtr)
                                        <tr>
                                            ...
                                            <td class="flex items-center justify-center">
                                                <a href="{{ url('/kriteria/' . $krtr->id . '/edit') }}"
                                                    title="Edit Kriteria"role="button" class="btn btn-info btn-sm">Edit</a>
                                                <button type="button" class="btn btn-error btn-sm ml-1"
                                                    onclick="showModal({{ $krtr->id }})">Hapus</button>
                                                <dialog id="my_modal" class="modal">
                                                    <div class="modal-box">
                                                        <p class="py-4">Konfirmasi hapus data Kriteria ini?</p>
                                                        <div class="modal-action">
                                                            <form id="deleteForm" method="POST"
                                                                action="{{ route('kriteria.destroy', $krtr->id) }}"
                                                ...
                                                <script>
                                                    function showModal(kriteriaId) {
                                                        ...
                                                        deleteForm.action = "{{ route('kriteria.destroy', '') }}" + '/' + kriteriaId;
                                                        modal.showModal();
                                                    }
