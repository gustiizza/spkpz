            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
              ...
              <div class="p-4 text-gray-900">
                <div class="overflow-x-auto">
                      ...
                      <form method="get" action="{{ route('penerima.lihat') }}">
                        <select name="kecamatan_id" class="select select-bordered fw-ull max-w-xs ml-2" id="kecamatan_id" onchange="this.form.submit()">
                          <option value="" @if(!$selectedKecamatan) selected @endif>Show All</option>
                          @foreach ($kecamatan as $kecamatan)
                            <option value="{{ $kecamatan->id }}" @if($selectedKecamatan == $kecamatan->id) selected 
                              @endif>{{ $kecamatan->nama }}
                            </option>
                          @endforeach
                        </select>
                      </form>
